...
{   //This class perfroms all activities on Xml files.
    internal class Xml_DDDFileHandling
    {
       ...
       */
        ...
        public static void FileDecode(InputRAWDataStorage ds, int blockCount, PrintingLayout pl, PrintingLayout.DriverCard hold, String fname)
        {
            ...
            CardIdentifier(ds.DataArchive[GetBlockId("EF ICC", ds)].Value, pl.cardICCI, xmlWriter);
            CardChipIdentification(ds.DataArchive[GetBlockId("EF IC", ds)].Value, pl.cardCI, xmlWriter);
            int CardType = ds.DataArchive[GetBlockId("EF Application_Identification", ds)].Value[0];
            switch (CardType)
            {
                case 1:
                    {
                        DriverCardInput(ds, hold, xmlWriter);
        ...
        public static void Report561_2006(PrintingLayout pl, PrintingLayout.DriverCard hold, string fname, DateTime repStartDate, DateTime repEndDate)
        {
            ...
            CardActivityDailyRecord[] cadrHERE = PrepareForCounting(hold, repStartDate, repEndDate);
            int ReportTimeSpan = (repEndDate - repStartDate).Days;
            Statistics stat = new Statistics();
            List<ShiftStats> lvr = ShiftFinesCounting(cadrHERE);
            GetWeeklyViolations(weeklyViolations, ReportTimeSpan, cadrHERE, stat);
            //printing elements
            foreach (var lvrlvr in lvr)
            {
                foreach (var itemsViolationRecord in lvrlvr.sv)
                {
                    printfine(xmlWriter, itemsViolationRecord);
                }
            }
