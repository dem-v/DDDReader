...
{ //This is an analytical module. It performs ananlysis of 561/2006 Decrete violations from recieved array.
  //Output is also a class variable.

    internal class ArrayManager
    {//was intended to become manager to host and flush ACTIVITIES array.
        public List<ActivityChangeInfoExtended> FlushActivityChangeInfos(CardActivityDailyRecord[] cadrHERE)
        {
            return null;
        }
    }
...
internal class QueueManager
    {//stub for queue manager, received from web.
    ...
    {   //TODO: Rebuild!!!
        ...
        {//This code intended to count all weekly violations (working week length etc.)
            ...
            foreach (var t in cadrHERE)
            {
                ...
                if (t.activityChangeInfo != null)
                {
                    for (int j = 0; j < t.activityChangeInfo.Length; j++)
                    {
                        ...
                        statsvar.vDrivingTimeSinceLastBreak += eventTime;
                        /*if ((eventTime > 270)||(statsvar.vDrivingTimeSinceLastBreak > 270)) {
                         * statsvar.isDrivingTimeNotIncludesShortBreak = true; 
        ...
        {//standard daily records are converted into activity change info extended, that is more comfortable to print
         //and contains all information about each activity inside it with explicit start and end time.
            ...
            var new_listoq = new List<ActivityChangeInfoExtended>();
            int new_listoqCounter = 0;
        ...
        //Check and delete duplicates
                ...
                if (i + 1 < listoq.Count)
                {
                    if (listoq[i].ActivityType == listoq[i + 1].ActivityType)
                    {
                        if ((listoq[i].DriverCardStatus == listoq[i + 1].DriverCardStatus) &&
                            (listoq[i].DrivingStatus == listoq[i + 1].DrivingStatus) &&
                            (listoq[i].SlotStatus == listoq[i + 1].SlotStatus))
                        {
                            var A = new ActivityChangeInfoExtended()
                            {
                                DriverCardStatus = listoq[i].DriverCardStatus,
                                DrivingStatus = listoq[i].DrivingStatus,
                                SlotStatus = listoq[i].SlotStatus,
                                ActivityType = listoq[i].ActivityType,
                                StartingDateTime = listoq[i].StartingDateTime,
                                EndingDateTime = listoq[i + 1].EndingDateTime,
                                Duration = listoq[i].Duration + listoq[i + 1].Duration
                            };
                            new_listoq.Add(A);
                            new_listoqCounter++;
                        }
            ...
            Console.WriteLine("Was {0} and became {1}, which is {2}!", listoq.Count, new_listoqCounter, listoq.Count - new_listoqCounter);
        ...
        public static List<ShiftStats> ShiftFinesCounting(CardActivityDailyRecord[] cadr)
        {
            ...
            var c = new Control();
            List<ActivityChangeInfoExtended> acie_in = aFaultProofListOfEvents(cadr); //call for conversion to CADR to ACIE

            //!!!Queue management was never debugged due to issues in getting a single shift separated!!! TODO: Implement and debug
        ...
        private static void PrintDebugShifts(List<ActivityChangeInfoExtended> ss, QueueManager queue, XmlWriter xml,
            int shiftcounter)
        {//print all shifts for debugging
            if (ss.Count < 1)
            {
            ...
            else
            {
                ...
                xml.WriteAttributeString("ShiftDurationVerbose",
                    Math.Truncate(B.Subtract(A).TotalHours).ToString("####") + ":" +
                    B.Subtract(A).Minutes.ToString("D"));
    ...
    {
        /*old method to verify faults per shift.
        ...
        */
        ...
        /* old variant of CADR -> ACIE types conversion. 
        ...
        */
        ...
        public int EnumerateLength(List<ActivityChangeInfoExtended> arr, int count)
        {//returns items count of ACIE list
            int x = 0;
            if (count == 0)
            {
                return 0;}
        ...
        {//Main code to extract shifts one by one. Its main problem is that it constantly keeps reducing or gathering too much data :(
            var singleShift = new List<ActivityChangeInfoExtended>();
            var i = 0;
//while (i < acie_array.Count - 1 && acie_array[i].ActivityType == "BREAK/REST") i++;
            var scoreForCrew = 0;

            #region CrewDrivingStatusIdentification
            //score like grade. works perfectly.
            ...
            //TODO:Make it to be 1440 and below!!!!
            ...
            if ((i < acie_array.Count) && (i>0))
            {
                        ...
                        .TotalMinutes > 1440)
                {
                    ...
                    try
                    {
                        ShiftDurationWithoutLast = EnumerateLength(acie_array, i);
                    }
                    catch (Exception)
                    {
                        ShiftDurationWithoutLast = 0;
                    }
        ...
        {//was intended to count all faults through the shift. It was never debugged. TODO: Implement and debug
